...
//#include "Vtop___024unit.h"				//包含一些自定义数据类型typedef，并经过Verilator转换后的头文件
...
int main(int argc, char** argv, char** env){
	...
	//nvboard_init();
	//reset(10);											//延迟十个时钟周期，看情况需要



	//对于组合逻辑，需要列出不同激励观察响应，下面的示例假设三输入
	top->en=0b0; top->x=0b0001; step_and_dump_wave();
	top->en=0b0; top->x=0b0010; step_and_dump_wave();
	top->en=0b0; top->x=0b0100; step_and_dump_wave();
	top->en=0b0; top->x=0b1000; step_and_dump_wave();
	top->en=0b1; top->x=0b0001; step_and_dump_wave();
	top->en=0b1; top->x=0b0010; step_and_dump_wave();
	top->en=0b1; top->x=0b0100; step_and_dump_wave();
	top->en=0b1; top->x=0b1000; step_and_dump_wave();
